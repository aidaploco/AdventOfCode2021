...
def flash_recursively(octopuses, i, j, flashed, counter):
    if octopuses[i][j] == -1:
        return counter
    if flashed[i][j] != 0:
        return counter
    ...
    counter += 1
    octopuses[i][j] = 0
    counter = flash_recursively(octopuses, i, j+1, flashed, counter)
    counter = flash_recursively(octopuses, i, j-1, flashed, counter)
    counter = flash_recursively(octopuses, i+1, j, flashed, counter)
    counter = flash_recursively(octopuses, i-1, j, flashed, counter)
    counter = flash_recursively(octopuses, i-1, j+1, flashed, counter)
    counter = flash_recursively(octopuses, i+1, j+1, flashed, counter)
    counter = flash_recursively(octopuses, i-1, j-1, flashed, counter)
    counter = flash_recursively(octopuses, i+1, j-1, flashed, counter)
    ...
    for _ in range(100):
        ...
        if len(np.where(octopuses == 0)[0]) > 0:
            for i, j in zip(np.where(octopuses == 0)[0], np.where(octopuses == 0)[1]):
                counter = flash_recursively(octopuses, i, j, flashed, counter)
